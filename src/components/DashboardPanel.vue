  ...
  <div class="dashboard-panel">
    ...
    <div class="panel-content">
      ...
      <div v-else-if="chartData" class="chart-container">
        ...
        <!-- Time Series -->
        <Line v-else-if="isTimeSeries" :data="chartData" :options="timeSeriesOptions" />
...
export default {
  ...
  methods: {
    ...
    processData(data) {
      ...
      // Check if data has multiple series (for stacked charts)
      const seriesMap = {};
      data.forEach(d => {
      ...
      if (seriesNames.length > 1 && !useThresholds) {
        ...
        const labels = [...new Set(data.map(d => {
          ...
          return this.isTimeSeries ? date : date.toLocaleDateString();
        }))];
        ...
        const datasets = seriesNames.map((seriesName, index) => {
          ...
          const values = labels.map(label => {
            const point = seriesData.find(d => {
              const date = new Date(d._time);
              const pointLabel = this.isTimeSeries ? date : date.toLocaleDateString();
              return String(pointLabel) === String(label);
            });
            return point ? point._value : 0;
          });
          
          const color = grafanaColors[index % grafanaColors.length];
          const opacity = fillOpacity / 100;
          
          return {
            label: seriesName,
            data: values,
            backgroundColor: this.isBarChart ? color : this.hexToRgba(color, opacity),
            borderColor: color,
            borderWidth: this.isBarChart ? 0 : 2,
            fill: this.isTimeSeries ? fillOpacity > 0 : !this.isBarChart,
            tension: 0.4
          };
        });
        ...
        if (useThresholds && thresholdSteps.length > 0) {
